# -*- coding: utf-8 -*-
"""可复用 HTML 组件生成函数"""
from config import RED, GREEN, DEEP_GREEN, CYAN, PURPLE, YELLOW, GOLD, CARD, BORDER, RADIUS, TEXT, TEXT_SUB


def pct_band(val, p5, p25, p75, p95):
    """根据分位数区间返回状态（指标都是"越高越好"型，彩虹色 5 档）

    - val < P5:    red        极低/异常
    - val < P25:   yellow     偏低/预警
    - val <= P75:  green      正常
    - val <= P95:  cyan       偏好
    - val > P95:   purple     极好/突出
    """
    if val < p5:
        return "red"
    elif val < p25:
...
def dot_span(band):
    c = {"green": GREEN, "cyan": CYAN, "purple": PURPLE, "yellow": YELLOW, "red": RED}[band]
    return f'<span style="color:{c};font-size:20px;margin-right:5px;">&#11044;</span>'


DOT_MAP = {"green": dot_span("green"), "cyan": dot_span("cyan"), "purple": dot_span("purple"), "yellow": dot_span("yellow"), "red": dot_span("red")}
LABEL_MAP = {"green": "正常", "cyan": "偏好", "purple": "极好", "yellow": "预警", "red": "异常"}
...
def kpi_card_with_bar(name, band, val, sub, current, p5, p25, p50, p75, p95, unit=""):
    ...
    vc = RED if current < p5 else (YELLOW if current < p25 else (PURPLE if current > p95 else (CYAN if current > p75 else GREEN)))
    def fmt(v):
        if unit == "%":
            return f"{v:.2f}%"
        if abs(v) >= 1_000_000:
    ...
    bar_html = (
        ...
        f'<div style="position:absolute;left:{p75_pct:.1f}%;width:{p95_pct - p75_pct:.1f}%;top:0;bottom:0;background:rgba(6,182,212,0.3);"></div>'
        f'<div style="position:absolute;left:{p95_pct:.1f}%;width:{100 - p95_pct:.1f}%;top:0;bottom:0;background:rgba(167,139,250,0.4);border-radius:0 3px 3px 0;"></div>'
    ...
    return (
        ...
        f'{sub}</div>'
        f'<div style="text-align:center;min-width:72px;border-left:1px solid {BORDER};padding-left:10px;">'
        f'{dot}<div style="font-size:10px;color:{TEXT_SUB};margin-top:2px;letter-spacing:0.05em;">{lbl}</div>'
        f'</div>'
        f'</div>'
        f'{bar_html}'
        f'</div>'
